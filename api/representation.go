	...
	if tr.Condition != nil {
		if tr.Condition.Services != nil {
			cond := &config.ServicesConditionConfig{
				UseAsModuleInput: tr.Condition.Services.SourceIncludesVar,
			}
			...
			tc.Condition = &config.ConsulKVConditionConfig{
				...
				UseAsModuleInput: tr.Condition.ConsulKv.SourceIncludesVar,
			}
			...
			tc.Condition = &config.CatalogServicesConditionConfig{
				CatalogServicesMonitorConfig: config.CatalogServicesMonitorConfig{
					...
					UseAsModuleInput: tr.Condition.CatalogServices.SourceIncludesVar,
	...
	if tc.Condition != nil {
		...
		case *config.ServicesConditionConfig:
			services := &oapigen.ServicesCondition{
				SourceIncludesVar: cond.UseAsModuleInput,
			}
			if len(cond.Names) > 0 {
