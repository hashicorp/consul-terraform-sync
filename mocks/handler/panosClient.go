...
func (_m *PanosClient) Commit(cmd interface{}, action string, extras interface{}) (uint, []byte, error) {
	...
	var r1 []byte
	var r2 error
	if rf, ok := ret.Get(0).(func(interface{}, string, interface{}) (uint, []byte, error)); ok {
		return rf(cmd, action, extras)
	}
...
func (_m *PanosClient) InitializeUsing(filename string, chkenv bool) error {
	...
	if len(ret) == 0 {
		panic("no return value specified for InitializeUsing")
	}
...
func NewPanosClient(t interface {
	mock.TestingT
	Cleanup(func())
}) *PanosClient {
	mock := &PanosClient{}
