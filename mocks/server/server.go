...
// TaskUpdate provides a mock function with given fields: ctx, conf, runOp
func (_m *Server) TaskUpdate(ctx context.Context, conf config.TaskConfig, runOp string) (bool, string, string, error) {
	ret := _m.Called(ctx, conf, runOp)

	var r0 bool
	if rf, ok := ret.Get(0).(func(context.Context, config.TaskConfig, string) bool); ok {
		r0 = rf(ctx, conf, runOp)
	} else {
		r0 = ret.Get(0).(bool)
	}
